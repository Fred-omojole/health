...
function TabIcon({ Icon, focused }: { Icon: React.ComponentType<{ className?: string; width?: number; height?: number }>; focused: boolean }) {
  return (
    <View className={`h-[38px] w-[38px] items-center justify-center rounded-2xl ${focused ? 'bg-[#F2733D]' : ''}`}>
      <Icon className={focused ? 'text-white' : 'text-[#8A93B2]'} width={22} height={22} />
    </View>
  );
}
...
function TabLabel({ focused, children }: { focused: boolean; children: string }) {
  return (
    <Text className={`mt-[14px] text-xs ${focused ? 'font-semibold text-[#F2733D]' : 'font-normal text-[#8A93B2]'}`}>
      {children}
    </Text>
  );
}
